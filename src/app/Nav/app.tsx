    ...
    <nav className="fixed top-0 w-full z-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-center h-20 sm:h-24 relative">
          <div
            className={`flex items-center justify-between w-full backdrop-blur-xl rounded-3xl border border-blue-100/50 px-6 transition-all duration-300 ${
              scrolled
                ? 'bg-white/85 shadow-xl shadow-blue-500/15'
                : 'bg-white/60 shadow-lg shadow-blue-500/10'
            }`}
          >
            {/* Logo */}
                ...
                return (
                  <Link key={item.id} href={item.href} className="group relative block">
                    <div className="relative bg-blue-50/80 rounded-2xl p-px overflow-hidden backdrop-blur-sm border border-blue-200/50 shadow-lg hover:shadow-xl transition-all duration-500 hover:scale-105 hover:border-blue-300/70">
                      ...
                      <span className="flex items-center justify-center gap-2 relative z-[1] bg-white/90 rounded-[15px] py-3 px-6 w-full group-hover:bg-white transition-all duration-300">
                      ...
                      {activeLink === item.id && (
                        <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 h-1.5 bg-gradient-to-r from-blue-500 to-blue-700 rounded-full shadow-lg shadow-blue-500/50" />
                      )}
                    </div>
                  </Link>
                );
                  ...
                  >
                    <IconComponent
                      ...
                      className={`transition-colors duration-300 ${
                        activeLink === item.id ? 'text-blue-600' : 'text-blue-500 group-hover:text-blue-600'
                      }`}
                    />
